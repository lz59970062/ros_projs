...
void oled_task(void *arg){
  ...
  while(1){
    ...
    for(int i=0;i<10;i++){
      battery_voltage = battery_voltage + analogRead(1)*3.3/4096.0*11.555/1.39;
    }
    ...
    memset(buffer+(SCREEN_WIDTH * ((SCREEN_HEIGHT + 7) / 8))/2, 0, (SCREEN_WIDTH * ((SCREEN_HEIGHT + 7) / 8))/2);
...
void oled_init() {
  ...
  // display.println("Hello ,I am starting");
  // display.display();  
