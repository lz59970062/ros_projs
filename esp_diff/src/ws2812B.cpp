...
uint8_t get_state()
{ // The purpose of this function is to determine the position of the highest set bit (the bit with the value of 1) in the binary representation of the input flag
  uint8_t state = 0;
  const uint8_t flag=led_state.flag;
  for (int i = 0; i < 8; i++)
  {
    if ((flag >> i) & 0x1)
    {
